// функция для отправки формы на указанный адрес
// idform - ID нашей формы
function submit_button(elem){ 
    // находим ID формы
    var idform=elem.id;    
    // создаем объект для отправки к PHP файлу
    var form_data = new FormData();
    // проверяем есть ли файлы
    if($('#sk_file','#'+idform).attr('type')){
        var file_data = $('#sk_file','#'+idform).prop('files')[0];
        form_data.append('file', file_data); 
    }
    // перебираем все элементы формы и добавляем значения в объект   
    $('#'+idform).find('input,select,textarea').each(function() {
        form_data.append($(this).attr('name'),$(this).val());
    });   
    // Отправляем AJAX запрос по указанному адресу с указанными данными
    $.ajax({
        cache: false,
        contentType: false,
        processData: false,
        // Метод по которому форма отправляет данные (Массив $_POST)
        type: "POST",
        // Адрес на который форма отправляет данные (РНР-файл)
        url: '/callback/post.php',
        // Переменные с данными которые отправляются формой
        data: form_data,
        // В случае успешного выполнения выводим данные из php файла
        success: function(text){ 
            text= Number(text);
            if(text == 1){
                $('#sk_call_form_container','#'+idform).html('<span class="sk_confirm_form">Ваше сообщение успешно отправлено!<br/> С вами свяжутся в ближайшее время</span>');
                setTimeout(fare, 1500);
            }else{
                $('#sk_call_error','#'+idform).text('Вы не подтвердили, что вы не робот');
                $('#sk_call_error','#'+idform).fadeIn();
            } 
        },
        // В случае ошибки выводим сообщение об ошибке
        error: function(){  
            alert('Произошла ошибка, проверте правильности установки формы и наличия файлов AJAX формы!');  
        }    
    }); 
}
function submit_buttonn(elem){ 
    // находим ID формы
    var idform=elem.id;    
    // создаем объект для отправки к PHP файлу
    var form_data = new FormData();
    ...
    $.ajax({
        ...
        success: function(text){ 
            text= Number(text);
            if(text == 1){
                $('#sk_call_form_container','#'+idform).html('<span class="sk_confirm_form rew">Ваш отзыв успешно отправлен!<br/> Спасибо за то что уделили нам время!</span>');
                setTimeout(fare, 1500);
            }else{
                $('#sk_call_error','#'+idform).text('Вы не подтвердили, что вы не робот');
                $('#sk_call_error','#'+idform).fadeIn();
...
function submit_buttonnn(elem){ 
    ...
    $.ajax({
        ...
        success: function(text){ 
            text= Number(text);
            if(text == 1){
                $('#sk_call_form_container','#'+idform).html('<span class="sk_confirm_form rew">Ваша заявка успешно отправлена!<br/> Спасибо за то что уделили нам время!</span>');
                setTimeout(fare, 1500);
            }else{
                $('#sk_call_error','#'+idform).text('Вы не подтвердили, что вы не робот');
                $('#sk_call_error','#'+idform).fadeIn();
...
// функция для МАСКИ НОМЕРА ТЕЛЕФОНА ВНИЗУ НУЖНО ПРОПИСАТЬ ИД-ШНИКИ
(function ($ss1) {
