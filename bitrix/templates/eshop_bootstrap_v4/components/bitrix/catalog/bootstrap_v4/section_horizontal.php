		...
		if ($isFilter): ?>
			<div class="row">
				<div class="col<?=(isset($arParams['FILTER_HIDE_ON_MOBILE']) && $arParams['FILTER_HIDE_ON_MOBILE'] === 'Y' ? ' d-none d-sm-block' : '')?>">
					<? $APPLICATION->IncludeComponent("bitrix:catalog.smart.filter", "super", array(
...
<div class="catalog-section-list">
	...
	<style>.catalog-section-list-tile-img-container a {background-image: none !important;border: none !important;padding: 0 !important;}
	a.catalog-section-list-item-link {
    text-align: left;
    display: block;
    text-decoration: none;
}
ul.catalog-section-list-tile-list.row.mb-4 li {
    flex: 0 0 6%;
}
a.catalog-section-list-item-link act {
    border: 1px solid #edd081;
    border-radius: 16px;
    padding: 5px;
    background: #edd081;
}
		...
		<div class="row">
			<div class="col">
				...
				if (ModuleManager::isModuleInstalled("sale"))
				{
					...
					{
						?>
						<div data-entity="parent-container" style="display:none;">
							<?
		...
		<div class="row">
			<div class="col">
<? if (CModule::IncludeModule("iblock")){$result1 = CIBlockSection::GetList(Array("sort"=>"asc"), Array('IBLOCK_ID'=>2,"CODE"=>array($arResult["VARIABLES"]["SECTION_CODE"]),'ACTIVE'=>'Y'), false, Array());;while($res1 = $result1->GetNext()){?>
<?if ($res1['DEPTH_LEVEL']==1){
$podrazs = $res1['IBLOCK_SECTION_ID'];
}else{
$podrazs = $res1['IBLOCK_SECTION_ID'];
}?>
<?}}?>
<?if (substr_count($_SERVER['REQUEST_URI'], 's-myasom')) {?>
<?$APPLICATION->IncludeFile('/include/blockkategory.php');?>
<?}elseif(substr_count($_SERVER['REQUEST_URI'], 's-kartofelem')) {?>
<?$APPLICATION->IncludeFile('/include/blockkategory.php');?>
<?}elseif(substr_count($_SERVER['REQUEST_URI'], 's_kuritsey')) {?>
<?$APPLICATION->IncludeFile('/include/blockkategory.php');?>
<?}elseif(substr_count($_SERVER['REQUEST_URI'], 's-syrom')) {?>
<?$APPLICATION->IncludeFile('/include/blockkategory.php');?>
<?}elseif(substr_count($_SERVER['REQUEST_URI'], 's-kapustoy')) {?>
<?$APPLICATION->IncludeFile('/include/blockkategory.php');?>
<?}elseif(substr_count($_SERVER['REQUEST_URI'], 's-ryboy')) {?>
<?$APPLICATION->IncludeFile('/include/blockkategory.php');?>
<?}elseif(substr_count($_SERVER['REQUEST_URI'], 's-gribami')) {?>
<?$APPLICATION->IncludeFile('/include/blockkategory.php');?>
<?}?>
				<? $APPLICATION->IncludeComponent("bitrix:catalog.section.list", "bootstrap_v4_v2", array(
						"IBLOCK_TYPE" => $arParams["IBLOCK_TYPE"],
						"IBLOCK_ID" => $arParams["IBLOCK_ID"],
						"SECTION_ID" => $podrazs,
						"SECTION_CODE" => $arResult["VARIABLES"]["SECTION_CODE"],
						...
						"SECTION_USER_FIELDS" => array("UF_NAMEPOD"),
				...
				$getFilter = str_replace('%20', ' ', $_GET['filter']);
				$GLOBALS['arFilter']=array('PROPERTY_NACHINKA_VALUE' => $getFilter);
				$intSectionID = $APPLICATION->IncludeComponent("bitrix:catalog.section", "bootstrap_v4", array(
						...
						"PRODUCT_QUANTITY_VARIABLE" => $arParams["PRODUCT_QUANTITY_VARIABLE"],
						"PRODUCT_PROPS_VARIABLE" => $arParams["PRODUCT_PROPS_VARIABLE"],
						"FILTER_NAME" => 'arFilter'/*$arParams["FILTER_NAME"]*/,
						"CACHE_TYPE" => $arParams["CACHE_TYPE"],
						"CACHE_TIME" => $arParams["CACHE_TIME"],
